...
#
# Each platform is added to PLATFORMS, which the `usage` target prints. That way
# the help text cannot drift out of sync with the targets that actually exist.
#
...
# its own target directory, so `make clean` has to visit each of them.
...
# toolchain.
#
# The tock submodule is only cleaned if it has been checked out; a repository
# cloned without --recursive has an empty tock/ directory and nothing to clean.
.PHONY: clean
clean:
	cargo clean
	rm -fr nightly/target/
	@for demo in $(DEMOS); do (cd "$$demo" && cargo clean) || exit 1; done
	@if [ -f tock/Makefile ]; then $(MAKE) -C tock clean; fi
